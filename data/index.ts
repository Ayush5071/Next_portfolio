export const navItems = [
  { name: "Home", link: "/" },
  { name: "Startups", link: "/#startups" },
  { name: "Projects", link: "/#personal-projects" },
  { name: "Hire Me", link: "/freelance" },
  { name: "Experience", link: "/#experience" },
  { name: "Impact", link: "/#extracurricular" },
];

export const gridItems = [
  {
    id: 1,
    title: "I believe in fostering teamwork and maintaining transparency in all communications.",
    description: "",
    className: "lg:col-span-3 md:col-span-6 md:row-span-4 lg:min-h-[60vh]",
    imgClassName: "w-full h-full",
    titleClassName: "justify-end",
    img: "/b1.svg",
    spareImg: "",
  },
  {
    id: 2,
    title: "I'm very flexible with time zone communications",
    description: "",
    className: "lg:col-span-2 md:col-span-3 md:row-span-2",
    imgClassName: "",
    titleClassName: "justify-start",
    img: "",
    spareImg: "",
  },
  {
    id: 3,
    title: "My tech stack",
    description: "I constantly try to improve",
    className: "lg:col-span-2 md:col-span-3 md:row-span-2",
    imgClassName: "",
    titleClassName: "justify-center",
    img: "",
    spareImg: "",
  },
  {
    id: 4,
    title: "Tech enthusiast with a passion for development.",
    description: "",
    className: "lg:col-span-2 md:col-span-3 md:row-span-1",
    imgClassName: "",
    titleClassName: "justify-start",
    img: "/grid.svg",
    spareImg: "/b4.svg",
  },
  {
    id: 5,
    title: "Building QuantsProgrammer - a learning platform that contains Roadmaps for various technologies.",
    description: "The Inside Scoop",
    className: "md:col-span-3 md:row-span-2",
    imgClassName: "absolute right-0 bottom-0 md:w-96 w-60",
    titleClassName: "justify-center md:justify-start lg:justify-center",
    img: "/b5.svg",
    spareImg: "/grid.svg",
  },
  {
    id: 6,
    title: "Do you want to start a project together?",
    description: "",
    className: "lg:col-span-2 md:col-span-3 md:row-span-1",
    imgClassName: "",
    titleClassName: "justify-center md:max-w-full max-w-60 text-center",
    img: "",
    spareImg: "",
  },
];

export const projects = [
  {
    id: 1,
    title: "Quants Programmer",
    des: "A full-stack learning platform providing structured roadmaps, interactive resources, and community support for mastering modern technologies.",
    detailedDesc: "Quants Programmer is a comprehensive platform designed to help learners master modern technologies through structured roadmaps, interactive resources, and a supportive community. It features a full-stack architecture and modern UI animations.",
    image: "/quantsprogrammer.jpg",
    skills: ["Next.js", "TypeScript", "MongoDB", "Node.js", "Express", "GSAP", "Tailwind CSS"],
    github_link: "https://github.com/Ayush5071/QuantsProgrammer",
    live: "https://quantsprogrammer.vercel.app",
    linkedin: "https://www.linkedin.com/posts/ayush-tiwari-84a823281_quantsprogrammer-turning-dreams-into-digital-activity-7279265377153335296-nMto?utm_source=share&utm_medium=member_desktop&rcm=ACoAAESkqvoBH6PYTNow6d35qEuxga6jIDGFx64",
    iframe: "https://www.linkedin.com/posts/ayush-tiwari-84a823281_quantsprogrammer-turning-dreams-into-digital-activity-7279265377153335296-nMto?utm_source=share&utm_medium=member_desktop",
    slideshow: ["/quantsprogrammer.jpg"]
  },

  {
    id: 3,
    title: "Green Club MNNIT",
    des: "Official website for Green Club MNNIT, featuring interactive animations and club activities.",
    detailedDesc: "The Green Club MNNIT website showcases club activities and events with engaging animations and a modern interface, built using GSAP and Framer Motion.",
    image: "/greenclub.svg",
    skills: ["GSAP", "Framer Motion", "Next.js", "TypeScript"],
    github_link: "https://github.com/Ayush5071/GreenClubMNNIT",
    live: "https://green-club-mnnit.vercel.app",
    linkedin: "https://www.linkedin.com/posts/ayush-tiwari-84a823281_greenclubmnnit-website-activity-7280000000000000001-yyyy?utm_source=share&utm_medium=member_desktop",
    iframe: "https://www.youtube.com/embed/greenclub-demo",
    slideshow: ["/greenclub.svg"]
  },
  {
    id: 2,
    title: "Paissa",
    des: "A fintech platform for seamless peer-to-peer payments, expense tracking, and financial analytics.",
    detailedDesc: "Paissa enables users to make peer-to-peer payments, track expenses, and analyze their finances with ease. Built with a modern stack and a focus on security and usability.",
    image: "/paissa.svg",
    skills: ["Next.js", "TypeScript", "MongoDB", "Node.js", "Tailwind CSS"],
    github_link: "https://github.com/Ayush5071/paissa",
    live: "https://paissa-9eml.vercel.app/",
    linkedin: "https://www.linkedin.com/posts/ayush-tiwari-84a823281_paissa-fintech-app-activity-7280000000000000000-xxxx?utm_source=share&utm_medium=member_desktop",
    iframe: "https://www.youtube.com/embed/paissa-demo",
    slideshow: ["/paissa.svg"]
  },
  {
    id: 4,
    title: "CareConnect AI",
    des: "A healthcare platform using AI for patient management, appointment scheduling, and health analytics.",
    detailedDesc: "CareConnect AI leverages artificial intelligence to streamline patient management, appointment scheduling, and health analytics for healthcare providers.",
    image: "/careconnectai.svg",
    skills: ["Next.js", "TypeScript", "MongoDB", "Node.js", "AI", "Tailwind CSS"],
    github_link: "https://github.com/Ayush5071/IOT_HEALTH",
    live: "#",
    linkedin: "https://www.linkedin.com/posts/ayush-tiwari-84a823281_careconnectai-healthcare-activity-7280000000000000003-wwww?utm_source=share&utm_medium=member_desktop",
    iframe: "https://www.youtube.com/embed/careconnectai-demo",
    slideshow: ["/careconnectai.svg"]
  },
  {
    id: 5,
    title: "Legal AI",
    des: "An AI-powered platform for legal document analysis and case prediction, leveraging NLP and machine learning.",
    detailedDesc: "Legal AI uses NLP and machine learning to analyze legal documents and predict case outcomes, making legal research more efficient.",
    image: "/legalai.svg",
    skills: ["Next.js", "TypeScript", "Python", "NLP", "Tailwind CSS"],
    github_link: "https://github.com/Ayush5071/Legal_AI",
    live: "#",
    linkedin: "https://www.linkedin.com/posts/ayush-tiwari-84a823281_legalai-platform-activity-7280000000000000002-zzzz?utm_source=share&utm_medium=member_desktop",
    iframe: "https://www.youtube.com/embed/legalai-demo",
    slideshow: ["/legalai.svg"]
  },
  {
    id: 5.5,
    title: "Hack36 College Hackathon Website",
    des: `Built the official Hack36 college hackathon website with Next.js, TypeScript, GSAP, and Three.js. Collaborated with seniors and mentors, learned teamwork, and handled tight deadlines. Special thanks to designer Prakhar for creative inputs.\n\n<small>Certificate available below.</small>`,
    detailedDesc: "The Hack36 website was built for a major college hackathon, featuring creative animations and a collaborative development process. Includes a certificate of participation.",
    image: "/hack36.svg",
    skills: ["Next.js", "TypeScript", "Tailwind CSS", "GSAP", "Three.js"],
    live: "https://hack36-8-0-ten.vercel.app/",
    linkedin: "https://www.linkedin.com/posts/ayush-tiwari-84a823281_hack36-college-hackathon-website-activity-7280000000000000004-xxxx?utm_source=share&utm_medium=member_desktop",
    iframe: "",
    slideshow: ["/hack36.svg"],
    certificate: {
      url: "https://drive.google.com/file/d/1ygijYd9x2Tns8enzsDQSmYjM0po3qlNY/view?usp=sharing",
      label: "View Certificate"
    }
  },
  {
    id: 6,
    title: "CampusX: College OlX",
    des: "An application designed for creating ads, facilitating live auctions, and enabling real-time chat to assist college users.",
    detailedDesc: "CampusX is a platform for college students to create ads, participate in live auctions, and chat in real time, enhancing campus commerce and communication.",
    image: "/campusx.jpg",
    skills: ["Next.js", "Express", "MongoDB", "Tailwind CSS", "Node.js"],
    github_link: "https://github.com/Ayush5071/Web_Stark",
    live: "https://www.linkedin.com/posts/ayush-tiwari-84a823281_campus-x-the-winning-project-of-innodev-activity-7263876541317603328-2cGr?utm_source=share&utm_medium=member_desktop&rcm=ACoAAESkqvoBH6PYTNow6d35qEuxga6jIDGFx64",
    slideshow: ["/campusx.jpg"]
  },
  {
    id: 7,
    title: "Real Time Chat App",
    des: "Developed a Real time chat app using Socket.io ",
    detailedDesc: "A real-time chat application built with Socket.io, supporting instant messaging and group chats with a modern backend.",
    image: "/p4.jpg",
    skills: ["Express", "Node.js", "MongoDB", "JavaScript", "Socket.io"],
    github_link: "https://github.com/Ayush5071/chatApp",
    live: "https://www.linkedin.com/posts/ayush-tiwari-84a823281_webdevelopment-realtimechat-socketio-activity-7236120270309105664-dxlu?utm_source=share&utm_medium=member_desktop",
    slideshow: ["/p4.jpg"]
  },
  {
    id: 8,
    title: "KISAAN - Farm Management System",
    des: "A farm management system using Express, Node.js, and MongoDB, integrated with an NLP chatbot using the bag-of-words model.",
    detailedDesc: "KISAAN is a farm management system that helps farmers manage resources and interact with an integrated NLP chatbot for support.",
    image: "/kisaan.jpg",
    skills: ["Express", "Node.js", "MongoDB", "JavaScript"],
    github_link: "https://github.com/Ayush5071/Agro_Techies",
    live: "https://www.linkedin.com/posts/ayush-tiwari-84a823281_empowering-kisaans-with-next-gen-agritech-activity-7189592418441216000-oWNh?utm_source=share&utm_medium=member_desktop",
    slideshow: ["/kisaan.jpg"]
  },
  {
    id: 9,
    title: "E-commerce Website",
    des: "A fully featured e-commerce website with a user-friendly interface and secure payment integration.",
    detailedDesc: "A robust e-commerce platform with secure payment integration and a seamless user experience, built with the MERN stack.",
    image: "/p4.svg",
    skills: ["JavaScript", "Node.js", "MongoDB", "Express"],
    github_link: "https://github.com/Ayush5071/Pixel-Paladins",
    live: "https://www.linkedin.com/posts/ayush-tiwari-84a823281_hogwarts-haul-where-fantasy-meets-reality-activity-7177179704649555968-AMnW?utm_source=share&utm_medium=member_desktop",
    slideshow: ["/p4.svg"]
  },
  {
    id: 10,
    title: "Video Transcoding App",
    des: "Developed a video transcoding app using FFmpeg that uploads a video and breaks it into chunks.",
    detailedDesc: "A video transcoding application that uploads videos and processes them into chunks using FFmpeg, supporting scalable video delivery.",
    image: "/p5.png",
    skills: ["Express", "Node.js", "MongoDB", "JavaScript", "FFmpeg", "Sockets"],
    github_link: "https://github.com/Ayush5071/Video-Transcoding-ffmpeg",
    live: "https://www.linkedin.com/posts/ayush-tiwari-84a823281_excited-to-unveil-my-latest-creation-activity-7253086498621644800-UayK?utm_source=share&utm_medium=member_desktop&rcm=ACoAAESkqvoBH6PYTNow6d35qEuxga6jIDGFx64",
    slideshow: ["/p5.png"]
  },
];


export const workExperience = [
  {
    id: 7,
    title: "9th Rank in Hack36-8.0",
    desc: "Secured 9th rank among 100+ teams from different colleges for creating CareConnect AI. Certificate available.",
    className: "md:col-span-2",
    link: "https://drive.google.com/file/d/1bFqxIk8VTeUN2YuGCsqoWL9pjgYBdDJE/view?usp=sharing"
  },
  {
    id: 1,
    title: "Winner of Innodev 2k25",
    desc: "Achieved 1st place among 80+ teams in a web development competition at NIT Allahabad.",
    className: "md:col-span-2",
    link: "https://www.linkedin.com/posts/ayush-tiwari-84a823281_thrilled-to-share-my-achievements-in-avishkar-activity-7264005934639644672-6nEq",
  },
  {
    id: 2,
    title: "Third Prize in Quintathalon 2k25",
    desc: "Secured 3rd place in a mock placement event, which included an online assessment and technical interview.",
    className: "md:col-span-2",
    link: "https://www.linkedin.com/posts/ayush-tiwari-84a823281_thrilled-to-share-my-achievements-in-avishkar-activity-7264005934639644672-6nEq",
  },
  {
    id: 3,
    title: "Winner of Dev or Die 2k24",
    desc: "Won the annual technical event for full-stack web development at NIT Allahabad.",
    className: "md:col-span-2",
    link: "https://www.linkedin.com/posts/ayush-tiwari-84a823281_thrilled-to-share_my-achievements-in-avishkar-activity-7264005934639644672-6nEq",
  },
  {
    id: 4,
    title: "Third Prize in Quintathalon 2k24",
    desc: "Awarded 3rd place in the annual mock placement event at NIT Allahabad, designed to simulate real placement processes.",
    className: "md:col-span-2",
    link: "https://www.linkedin.com/posts/ayush-tiwari-84a823281_thrilled-to-share_my-achievements-in-avishkar-activity-7264005934639644672-6nEq",
  },
  {
    id: 5,
    title: "Leetcode Rank 1230",
    desc: "Ranked 1230 out of 32,000 participants, achieving a maximum rating of 1760.",
    className: "md:col-span-2",
    link: "https://www.linkedin.com/posts/ayush-tiwari-84a823281_thrilled-to-share_my-achievements-in-avishkar-activity-7264005934639644672-6nEq",
  },
  {
    id: 6,
    title: "10k Followers on LinkedIn and 1k Subscribers on YouTube",
    desc: "Gained a significant following by sharing my tech journey and projects, reaching 10k followers on LinkedIn and 1k subscribers on YouTube.",
    className: "md:col-span-2",
    link: "https://www.linkedin.com/posts/ayush-tiwari-84a823281_thrilled-to-share_my-achievements-in-avishkar-activity-7264005934639644672-6nEq",
  },
];

export const Experiences = [
  {
    id: 1,
    title: "Software Developer Intern",
    desc: "Built Cipher, a payroll management system for Computer Software Solutions, Kolkata.",
    detailedDesc: "Developed Cipher — a full-featured payroll management system serving as both a desktop application and mobile app. Built using React Native for mobile and Electron.js for the desktop client. Implemented employee management, salary processing, attendance tracking, and financial reporting modules.",
    skills: ["React Native", "Electron.js", "Node.js", "Express", "MongoDB"],
    github: "#",
    linkedin: "#",
    video: "",
    role: "Full Stack Developer — Computer Software Solutions, Kolkata",
    image: "/projects/cipher.jpg",
    slideshow_images: [],
  }
];

export const extracurricularActivities = [
  {
    title: "My Startups",
    activities: [
      {
        name: "PrepSutra.tech",
        role: "Founder",
        duration: "2024 - Present",
        description: "Platform helping college freshers learn tech stacks, coding, and placement prep. 800+ active users. Built entirely solo."
      },
      {
        name: "Mentix.in",
        role: "Founder",
        duration: "2024 - Present",
        description: "Courses & test series for JEE students. 500+ active users. Solo product — built from scratch."
      }
    ]
  },
  {
    title: "Content & Community",
    activities: [
      {
        name: "LinkedIn",
        role: "Tech Content Creator",
        duration: "2022 - Present",
        description: "13k+ followers — regularly posting projects, tech insights, and development updates."
      },
      {
        name: "YouTube — JEE Mentor",
        role: "Content Creator",
        duration: "2022 - Present",
        description: "1.3k subscribers — creating tutorials, project walkthroughs, and JEE mentoring videos."
      }
    ]
  },
  {
    title: "Skills & Speaking",
    activities: [
      {
        name: "Technical Skills",
        role: "Builder",
        duration: "",
        description: "Generative AI, Web Development, React Native, Next.js, Electron.js, Node.js, TypeScript"
      },
      {
        name: "Public Speaking",
        role: "Orator",
        duration: "",
        description: "Very good orator — conducted briefings for 800+ students and led multiple college-level events."
      }
    ]
  }
];
